...
interface CommentStyle {
  start: string;
  end: string; // '' for line comments, e.g. ' */' for block comments
}

// Built-in defaults, keyed by VS Code languageId.
// Anything not listed falls back to "//".
const DEFAULT_STYLES: Record<string, CommentStyle> = {
  python: { start: '#', end: '' },
...
function getCommentStyle(languageId: string): CommentStyle {
  const overrides = vscode.workspace
    ...
    .get<Record<string, CommentStyle>>('languageComments', {});
...
function getTargetWidth(document: vscode.TextDocument): number {
  const config = vscode.workspace.getConfiguration('commentBanner', document.uri);
  const configured = config.get<number>('lineLength', 0);
...
function extractLabel(lineText: string, style: CommentStyle): string {
